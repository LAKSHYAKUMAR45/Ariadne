...
CREATE TABLE IF NOT EXISTS encrypted_blobs (
  ...
  CONSTRAINT encrypted_blobs_id_team_type_key UNIQUE (id, team_id, blob_type)
);
...
CREATE TABLE IF NOT EXISTS task_file_capture_entries (
  ...
  CONSTRAINT task_file_capture_entries_snapshot_fk
    FOREIGN KEY (snapshot_blob_id, team_id, snapshot_blob_type)
    REFERENCES encrypted_blobs (id, team_id, blob_type)
    ON DELETE RESTRICT,
  CONSTRAINT task_file_capture_entries_diff_fk
    FOREIGN KEY (diff_blob_id, team_id, diff_blob_type)
    REFERENCES encrypted_blobs (id, team_id, blob_type)
    ON DELETE RESTRICT
);
...
-- rewritten or erased by application-level access.
